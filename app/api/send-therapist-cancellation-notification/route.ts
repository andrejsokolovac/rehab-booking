...
function getRequestPayload(
  ...
  if (fields.length !== 1 || fields[0] !== "cancelToken") {
    return null;
  }
  ...
  return UUID_PATTERN.test(cancelToken) ? { cancelToken } : null;
}
...
function createCancellationEmailHtml(
        ...
        <div style="padding:32px 16px;">
          <div style="max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #e1e8e5;border-radius:24px;overflow:hidden;">
            <div style="background:#b45745;padding:28px 32px;color:#ffffff;">
              ...
              <h1 style="margin:14px 0 0;font-size:28px;line-height:1.25;">Termin je otkazan</h1>
            </div>
            <div style="padding:32px;">
              <p style="margin:0 0 24px;font-size:16px;line-height:1.7;color:#526b66;">Poštovani/a ${safeTherapistName}, sledeći termin je otkazan.</p>
              <table role="presentation" style="width:100%;border-collapse:collapse;font-size:16px;">
                ...
                <tr>
                  <td style="padding:12px 0;border-top:1px solid #edf1ef;color:#6b807c;">Prvobitni početak</td>
                  <td style="padding:12px 0;border-top:1px solid #edf1ef;text-align:right;font-weight:700;">${appointmentStartTime}</td>
                </tr>
                <tr>
                  <td style="padding:12px 0;border-top:1px solid #edf1ef;color:#6b807c;">Prvobitni kraj</td>
...
export async function POST(request: Request) {
  ...
  if (!payload) {
    return Response.json(
      { success: false, error: "Zahtev ne sadrži ispravan token." },
      { status: 400 },
    );
  }
  ...
  const { data: appointment, error: appointmentError } = await supabaseAdmin
    .from("appointments")
    .select(
      "id, therapist_id, service_id, child_name, start_at, end_at, status, therapist_cancellation_email_sent_at",
    )
  ...
  if (appointment.therapist_cancellation_email_sent_at !== null) {
    return Response.json({
      success: true,
      skipped: true,
      reason: "already_sent",
    ...
    const response = await resend.emails.send(
      {
        ...
        html: createCancellationEmailHtml(therapistName, {
          childName,
          serviceName,
          ...appointmentDateTime,
        }),
      ...
      {
        idempotencyKey: `therapist-cancellation/${appointmentId}`,
      },
    );
  ...
  const { error: updateError } = await supabaseAdmin
    .from("appointments")
    .update({ therapist_cancellation_email_sent_at: new Date().toISOString() })
    .eq("id", appointmentId)
    .is("therapist_cancellation_email_sent_at", null);
