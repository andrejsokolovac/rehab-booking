...
type CancellationNotificationRequest = {
...
type ParentNotificationDetails = AppointmentDateTime & {
  childName: string;
...
function getRequestPayload(
  body: unknown,
): CancellationNotificationRequest | null {
...
function createCancellationEmailHtml(details: ParentNotificationDetails) {
      ...
      <body style="margin:0;background:#fffaf3;color:#243c38;font-family:Arial,sans-serif;">
        <div style="padding:32px 16px;">
          <div style="max-width:600px;margin:0 auto;background:#ffffff;border:1px solid #e1e8e5;border-radius:24px;overflow:hidden;">
            <div style="background:#b45745;padding:28px 32px;color:#ffffff;">
              ...
              <h1 style="margin:14px 0 0;font-size:28px;line-height:1.25;">Vaš termin je otkazan</h1>
            </div>
            <div style="padding:32px;">
              <p style="margin:0 0 24px;font-size:16px;line-height:1.7;color:#526b66;">Obaveštavamo vas da je centar otkazao sledeći termin.</p>
              <table role="presentation" style="width:100%;border-collapse:collapse;font-size:16px;">
                ...
                <tr>
                  <td style="padding:12px 0;border-top:1px solid #edf1ef;color:#6b807c;">Prvobitni datum</td>
                  <td style="padding:12px 0;border-top:1px solid #edf1ef;text-align:right;font-weight:700;">${appointmentDate}</td>
                </tr>
                <tr>
                  <td style="padding:12px 0;border-top:1px solid #edf1ef;color:#6b807c;">Prvobitno vreme</td>
              ...
              <div style="margin-top:28px;padding-top:24px;border-top:1px solid #edf1ef;">
                <h2 style="margin:0 0 10px;font-size:18px;line-height:1.4;color:#243c38;">Adresa centra</h2>
                <p style="margin:0;font-size:16px;line-height:1.7;color:#526b66;">${centerAddress}</p>
              </div>
            </div>
          </div>
        </div>
...
export async function POST(request: Request) {
  ...
  const { data: appointment, error: appointmentError } = await supabaseAdmin
    .from("appointments")
    .select(
      "id, therapist_id, service_id, child_name, email, start_at, end_at, status, parent_cancellation_email_sent_at",
    )
  ...
  if (appointment.parent_cancellation_email_sent_at !== null) {
    return Response.json({
      success: true,
      skipped: true,
      reason: "already_sent",
  ...
  try {
    const response = await resend.emails.send(
      ...
      {
        idempotencyKey: `parent-cancellation/${appointmentId}`,
      },
    );
  ...
  if (!emailWasSent) {
    return Response.json(
      {
        success: false,
        error: "Obaveštenje roditelju trenutno nije moguće poslati.",
      },
      { status: 502 },
    );
  }
  ...
  const { error: updateError } = await supabaseAdmin
    .from("appointments")
    .update({ parent_cancellation_email_sent_at: new Date().toISOString() })
    .eq("id", appointmentId)
    .eq("status", "cancelled")
    .is("parent_cancellation_email_sent_at", null);
